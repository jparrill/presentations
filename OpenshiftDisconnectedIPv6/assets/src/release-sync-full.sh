...
# Variables
export PULL_SECRET_JSON=/root/pull_secret.json
...
export HTTP_PROXY=proxy.acme.lab:3128
export HTTPS_PROXY=proxy.acme.lab:3128
export NO_PROXY=bastion1.acme.lab
...
function download_rhcos() {
	...
	if [[ ! -d ${OCP_RELEASE_DOWN_PATH} ]]; then
		...
		sudo curl -x $HTTP_PROXY -s -L -o ${OCP_RELEASE_DOWN_PATH}/$(echo $RHCOS_QEMU_URI | xargs basename) ${RHCOS_QEMU_URI}
		echo "--> Downloading RHCOS resources: RHCOS Openstack Image"
		sudo curl -x $HTTP_PROXY -s -L -o ${OCP_RELEASE_DOWN_PATH}/$(echo $RHCOS_OPENSTACK_URI | xargs basename) ${RHCOS_OPENSTACK_URI}
		echo "--> Downloading RHCOS resources: RHCOS ISO"
		sudo curl -x $HTTP_PROXY -s -L -o ${OCP_RELEASE_DOWN_PATH}/$(echo $RHCOS_ISO_URI | xargs basename) ${RHCOS_ISO_URI}
		echo "--> Downloading RHCOS resources: RHCOS RootFS"
		sudo curl -x $HTTP_PROXY -s -L -o ${OCP_RELEASE_DOWN_PATH}/$(echo $RHCOS_ROOT_FS | xargs basename) ${RHCOS_ROOT_FS}
	else
